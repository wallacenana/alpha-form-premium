...
function register_form_depois_do_envio_controls(Widget_Base $widget)
{
    ...
    if (!$api_key || !$api_url) {
        $widget->add_control(
            ...
            [
                'type' => Controls_Manager::RAW_HTML,
                'raw' => '<strong>⚠️ Configure sua API Key do Active nas configurações do Alpha Form.</strong>',
    ...
    $api_key_chimp = get_option('alpha_form_mailchimp_api_key');
    ...
    if (!$api_key_chimp) {
        $widget->add_control(
            ...
            [
                'type' => Controls_Manager::RAW_HTML,
                'raw' => '<strong>⚠️ Configure sua API Key e Audience ID do Mailchimp nas configurações do Alpha Form.</strong>',
                'content_classes' => 'elementor-panel-alert elementor-panel-alert-warning',
            ]
        );
    } else {
        ...
        $server_prefix = explode('-', $api_key_chimp)[1] ?? '';

        if (!$server_prefix) return;

        $mailchimp  = new \MailchimpMarketing\ApiClient();
        $mailchimp->setConfig([
            'apiKey' => $api_key_chimp,
